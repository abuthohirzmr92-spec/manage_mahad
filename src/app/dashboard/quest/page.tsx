...
export default function QuestPage() {
  ...
  const filteredQuests = mockQuest.filter((q) => {
  ...
  const questAktif = mockQuest.filter((q) => q.status === 'in_progress').length;
  const questSelesai = mockQuest.filter((q) => q.status === 'completed').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Quest Aktif" value={questAktif} icon={Trophy} iconClassName="bg-amber-500/10 text-amber-600 dark:text-amber-400" />
        ...
        <StatsCard title="Quest Selesai" value={questSelesai} icon={CheckCircle} iconClassName="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" />
                ...
                <tbody className="divide-y divide-border">
                  {filteredQuests.map((q) => (
                    <tr key={q.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {/* Remapping existing status to match new workflow visually if needed, but we use existing logic + new variants */}
