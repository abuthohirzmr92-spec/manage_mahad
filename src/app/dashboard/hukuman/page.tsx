...
import { mockHukuman } from '@/data/mock';
...
export default function HukumanPage() {
  const aktif = mockHukuman.filter((h) => h.status === 'aktif').length;
  const selesai = mockHukuman.filter((h) => h.status === 'selesai').length;
  const dibatalkan = mockHukuman.filter((h) => h.status === 'dibatalkan').length;
